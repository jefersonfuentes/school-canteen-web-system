...
<body>
	...
	<main class="d-flex">
		...
		<section class="mt-4 w-75 mx-auto mobile-target">
			<h1 class="fs-3">
				<?php if ($estado == 1) echo 'Administradores';
				else echo 'Administradores Eliminados' ?>
			</h1>
			<section class="table-system mt-3">
				<div class="d-flex justify-content-between gap-1">
					...
					<div class="d-flex justify-content-between gap-1">
						<?php if ($estado == 1) { ?>
							<button onclick="CrearAdministradores()" class="btn text-white py-2 table__green-button" title="Crear nuevo"><i class="fa-solid fa-plus"></i></button>
							<button onclick="CambiarEstado(0, 1)" class="btn text-white py-2 table__red-button" title="Eliminar"><i class="fa-solid fa-trash"></i></button>
	...
	<form id="formModificar" action="./?dir=admin&controlador=Funcionario&accion=VistasAdmin&id=modificar" method="post" hidden>
	...
	<div id="datos" hidden data-administradores='<?php echo $administradores ?>' data-estadoUsuarios='<?php echo $estado ?>'></div>
	<script>
		...
		if (datos.dataset.administradores != "") {
			arrayUsuarios = JSON.parse(datos.dataset.administradores);
			buscarEnTabla("", ESTADO_USUARIOS);
		}

		function verActivos() {
			location.href = "./?dir=admin&controlador=Funcionario&accion=VistasAdmin&id=main";
		}

		function verInactivos() {
			location.href = "./?dir=admin&controlador=Funcionario&accion=VistasAdmin&id=main&estados=0";
		}

		function CrearAdministradores() {
			location.href = "./?dir=admin&controlador=Funcionario&accion=VistasAdmin&id=crear";
		}

		const formModificar = document.getElementById('formModificar');

		function ModificarAdministradores(...datosEntidad) {
		...
		function LlenarInfoModal(...datosEntidad) {
			...
			let estado;
			if (datosEntidad[4] === "1") estado = "Administrador Activo";
			else estado = "Administrador Eliminado";
		...
		function CambiarEstado(estado) {
			...
			if (rutaValida) {
				let direccionamiento = `./?dir=admin&controlador=Funcionario&accion=CambiarEstado&id[]=${estado}&id[]=1`;
				...
				`
					<tr class="transicion align-middle" data-id="${e.id}">
							...
							<td class="text-center">
								<i class="fa-solid fa-pen-to-square me-1 fs-5" style="cursor: pointer" onclick="ModificarAdministradores(${e.id}, '${e.nombre}', '${e.apellido1}', '${e.apellido2}', '${e.correo}', '${e.estado}')"></i>
	...
	if (isset($_REQUEST['alerta'])) {
		...
		if ($nombreAlerta == "success") {
			...
			echo "<script>alertify.warning('Algunos administradores no se han creado');</script>";
		}
	}
	?>
</body>
