...
<body>
	...
	<main class="d-flex">
		<?php echo $sidebar; ?>
		<div class="w-50 mx-auto mt-4 rounded general-shadow mobile-target overflow-hidden">
			...
			<form id="manualForm" action="./?dir=admin&controlador=Profesor&accion=Crear" method="POST" class="px-4 pb-4" enctype="multipart/form-data">
				<h2 class="fs-3">Nuevo Profesor</h2>
				...
				<div class="mb-3">
					<label for="inputFotoPerfil" class="form-label">Foto de Perfil (Opcional)</label>
			...
			<form class="d-none px-4 pb-4" id="excelForm">
				<div class="d-flex flex-wrap justify-content-between">
					<h2 class="fs-3 mb-0">Ingresar Profesores</h2>
					<button type="button" id="btnPlantilla" class="btn btn-comedor">Descargar Plantilla</button>
				</div>
				<div class="mb-4 mt-3 position-relative">
					<label for="formFileLg" class="form-label d-block">Arrastra o selecciona un archivo de excel con los profesores</label>
	...
	<table hidden id="tabla">
		...
		<tbody>
			<tr>
				...
				<th>García</th>
				<th>Mora</th>
				<th>juan@gmail.com</th>
			</tr>
		</tbody>
	</table>

	<form id="formEnviarExcel" action="./?dir=admin&controlador=Profesor&accion=CrearPorJSON" method="post" hidden>
	...
	<script>
		...
		enviarExcelButton.addEventListener('click', (e) => {
			const formEnviarExcel = document.getElementById('formEnviarExcel');
		...
		btnPlantilla.addEventListener("click", function() {
			let tableExport = new TableExport(tabla, {
				exportButtons: false,
				filename: "PlantillaProfesores",
				sheetname: "PlantillaProfesores",
			});
